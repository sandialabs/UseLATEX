...
\newcommand{\UseLATEXVersion}{2.8.0}
...
\begin{document}
  ...
  \UseLATEX is available at this repo:
  \begin{quote}
...
add_latex_document(MyDoc.tex
  ...
  If you do not have ImageMagick, you can get it for free from \url{http://www.imagemagick.org}.
...
add_latex_document(UberReport.tex
  ...
  \subsection{Specifying Command Line Arguments}
  \label{sec:SpecifyingCommandLineArguments}

  It is usually safest to let \UseLATEX set the arguments to the \latex commands for you.
  However, there are some rare cases where you may need to add your own command line options.
  For example, you may need to add a flag like \textcmake{-shell-escape} or \textcmake{--enable-write18} to enable some packages (see Section \ref{sec:How_do_I_resolve_write_18_errors_with_pstricks_or_pdftricks} for details).

  One way to add flags to the \latex compile is to add them to the \textcmakevar{LATEX\_COMPILER\_FLAGS} CMake variable.
  This is particularly useful if you want to add a flag temporarily that does not always need to be set.
  For example, you could add \textcmake{-draftmode} to turn on draft mode and suppress the final output (for testing purposes).

  However, this option is tedious and confusing if this is an option that has to be added every time to make the build work.
  If you want your \textfile{CMakeLists.txt} to automatically add this option every time, you can do so using the \textcmake{COMPILER\_FLAGS} target property.
  This property is attached to the \textmaketarget{\emph{$\langle$doc$\rangle$}\_dvi} or \textmaketarget{\emph{$\langle$doc$\rangle$}\_pdf} target, where \textmaketarget{\emph{$\langle$doc$\rangle$}} is the name of the \latex document being created (as described in Section \ref{sec:BuldingMultipleLatexDocuments}).
  The text in this property is added to the command line when executing \textprog{latex} or \textprog{pdflatex}.

  This target property can be added using the \textcmake{set\_target\_properties} CMake function.
  Here is an example that adds \textcmake{--enable-write18} to both the dvi and pdf builds.

  \begin{CodeListing}
add_latex_document(HelloWorld.tex)

set_target_properties(HelloWorld_dvi HelloWorld_pdf
  PROPERTIES COMPILER_FLAGS "--enable-write18"
  )
  \end{CodeListing}

  Be aware that different distributions of \latex may accept different command line arguments.
  By adding compiler flags this way, your \latex document may not compile correctly on machines that are not your own.
  ...
  (\url{http://www.cygwin.com/}) and the
  ...
  Most applications port fairly easily to Mac OS so long as you are willing to use them as typical Unix or X11 programs.
  To make things even easier, I recommend taking advantage of a Mac porting project to make this process even easier.
  \href{http://www.macports.org}{MacPorts} (\url{http://www.macports.org}) and \href{https://brew.sh/}{Homebrew} (\url{https://brew.sh/}) are both good tools for this purpose.
...
add_latex_document(MyDoc.tex
  ...
  Unfortunately, allowing \latex to run commands on your system is a security issue.
  Consequently, most versions of \latex disable this feature by default.
  Thus, to use packages that rely
  on \textlatex|\write18|, you may have to enable the feature via a command line option that allows the \textfile{.tex} document to execute arbitrary commands on your computer.
  This is typically done by adding a command line option, which is usually either \textcmake{--enable-write18} or \textcmake{-shell-escape}.

  Section \ref{sec:SpecifyingCommandLineArguments} describes how to pass custom flags to \latex.
  One way to do this is by adding \textcmake{--enable-write18} (or \textcmake{-shell-escape}) to the \textcmakevar{LATEX\_COMPILER\_FLAGS} CMake variable during configuration.
  This is easy to do and allows you to customize the argument based on what \latex distribution you are using.
  However, it also means you have to make this change every time you configure the project, and will be confusing to new users.

  An alternate way to do this is by adding \textcmake{--enable-write18} (or \textcmake{-shell-escape}) to the \textcmake{COMPILER\_FLAGS} target property for the \textmaketarget{*\_dvi} and/or \textmaketarget{*\_pdf} target.
...
set_target_properties(HelloWorld_dvi HelloWorld_pdf
  ...
  This addition could cause a failure for any \latex implementation that does not support the \textcmake{--enable-write18} flag (for which there are many).
  It is also allowing \latex to execute arbitrary commands on your system, so you might be introducing security vulnerabilities to anyone compiling your repo.

  See Section \ref{sec:SpecifyingCommandLineArguments} for more details on adding custom command line argument flags to the \latex build.
...
\includegraphics{my.image.pdf}
  ...
  As of \UseLATEX version 2.7.2 and CMake version 3.12, a target is added to the build to check whether the list of files in any of the \textcmake{IMAGE\_DIRS} have changed.
  Thus, this problem is much less likely to happen now, but it still can occur.
